"""
LJSpeechWHAM data preparation.
Download: LJSpeechWHAM.tar.gz
...
def prepare_ljspeechwham(
    ...
    """
    Prepares the csv files for the LJSpeechWHAM dataset.
    ...
    Example
    -------
    >>> data_folder = 'data/LJSpeechWHAM/'
    >>> save_folder = 'save/'
    >>> splits = ['train', 'valid']
    >>> split_ratio = [90, 10]
    >>> seed = 1234
    >>> prepare_ljspeechwham(data_folder, save_folder, splits, split_ratio, seed)
    """
    ...
    # This is VERY bad, it takes only a few seconds to regenerate the JSON files
    # If we skip preparation, we might have hardcoded path on another node or different split ratios
    ...
    # Match clean audios with noise
    noise_folder = wavs_folder.replace(WAVS, NOISE)
    assert os.path.exists(noise_folder), "noise/ folder does not exist"

    wavs_duration_file = os.path.join(wavs_folder, DURATIONSFILE)
    wavs_durations = {}
    with open(wavs_duration_file, encoding="utf-8") as f:
        _ = next(f)  # Skip header
        for line in f:
            filename, duration = line.split(",")
            wavs_durations[filename] = float(duration)

    noise_duration_file = os.path.join(noise_folder, DURATIONSFILE)
    noise_durations = {}
    with open(noise_duration_file, encoding="utf-8") as f:
        _ = next(f)  # Skip header
        for line in f:
            filename, duration = line.split(",")
            noise_durations[filename] = float(duration)

    # Sort by decreasing durations
    wavs_durations = dict(
        sorted(wavs_durations.items(), key=lambda item: item[1], reverse=True)
    )
    noise_durations = dict(
        sorted(noise_durations.items(), key=lambda item: item[1], reverse=True)
    )
    assert len(wavs_durations) == 13100
    assert len(noise_durations) == 13900

    # Read SNRs
    snr_file = os.path.join(data_folder, SNRFILE)
    with open(snr_file, encoding="utf-8") as f:
        snrs = [float(x) for x in f]
    assert len(snrs) == 13900

    # Do the matching
    clean2noise = {}
    for wav_filename, noise_filename, snr in zip(
        wavs_durations, noise_durations, snrs
    ):
        duration = wavs_durations[wav_filename]
        clean2noise[wav_filename] = (noise_filename, snr, duration)
...
def prepare_json(
    ...
    for index in tqdm(seg_lst):
        ...
        noise_filename, snr, duration = clean2noise[f"{id}.wav"]
        noise = os.path.join(noise_folder, noise_filename)
