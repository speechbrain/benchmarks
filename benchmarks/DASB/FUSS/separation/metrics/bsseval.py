...
class BSSEval(MetricStats):
    def __init__(
        self, n_sources, source_names=None, permutation_invariant=True
    ):
        """
        ...
        self.source_names = source_names or [
            f"Source {i + 1}" for i in range(n_sources)
        ]
    ...
    def compute_metrics(self, reference_sources, estimated_sources):
        ...
        try:
            sdr, sir, sar, perm = bss_eval_sources(
                reference_sources,
                estimated_sources,
                compute_permutation=self.permutation_invariant,
                load_diag=1e-5,
            )
            is_all_zeros = is_all_zeros[
                perm
            ]  # Apply permutation to silent mask
    ...
    def add(
        ...
        assert (
            reference_sources.shape[0] == self.n_sources
        ), "Mismatch in number of reference sources."
        assert (
            estimated_sources.shape[0] == self.n_sources
        ), "Mismatch in number of estimated sources."
    ...
    def pretty_print(self):
        ...
        for metric, stats in summary.items():
            ...
            for i, source_name in enumerate(self.source_names):
                print(
                    f"  {source_name}: Mean = {stats['mean'][i]:.2f}, Std = {stats['std'][i]:.2f}"
                )
