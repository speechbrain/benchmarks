...
def random_chunk_indices(total_samples, chunk_samples, num_chunks):
    ...
    if max_start <= 0:
        return [
            0
        ] * num_chunks  # Only one possible chunk if audio is shorter than chunk size
    return np.random.randint(0, max_start + 1, size=num_chunks)


def process_track(
    split, track, track_path, target_dir, chunk_size, num_chunks, required_files
):
    """
    ...
    start_indices = random_chunk_indices(
        total_samples, chunk_samples, num_chunks
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract random audio chunks from tracks in parallel."
    )
