...
class Separation(sb.Brain):
    ...
    def _get_encdec_helper(self, tokenizer_type):
    ...
    def sig_to_toks(self, sig, lens):
        return self.encdec.sig_to_toks(sig, lens)

    @torch.no_grad()
    def toks_to_sig(self, toks):
        return self.encdec.toks_to_sig(toks)
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            ...
            self.checkpointer.save_and_keep_only(
                meta={"TER": stage_stats["TER"], "epoch": epoch},
                min_keys=["TER"],
            ...
            if self.hparams.compute_metrics:
                stage_stats["BSSEval"] = self.bsseval_metric.summarize()
            self.hparams.train_logger.log_stats(
            ...
            if if_main_process():
                # Save dWER
                if self.hparams.compute_metrics:
                    self.bsseval_metric.write_stats(self.hparams.bsseval_file)
...
if __name__ == "__main__":
    ...
    # Dataset preparation
    from musdb_prepare import prepare_musdb as prepare_data
